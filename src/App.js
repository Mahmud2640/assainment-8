...
function App() {
  ...
  const AddingSelectedWatch = (selectedWatch) => {
    ...
    if (selectedWatches.length < 4) {
      ...
      setWarning({
        state: true,
        message: "Only 4 maximum items can be added!",
      });
    }
  };
  ...
  const removeSelectedWatch = (id) => {
    let restSelectedWatch = selectedWatches.filter((watch) => watch.id !== id);
    ...
    <>
      ...
      <section className="watches-box">
        <div className="container">
          <SectionTitle
            title="BD Watch Store"
            desc="Buy the best watch for yourself."
          />
          <div className="row">
                  ...
                  {watches.length > 0 &&
                    watches.map((watch) => (
                      <Watch
                        key={watch.id}
                        watch={watch}
                        AddingSelectedWatch={AddingSelectedWatch}
                      />
                    ))}
                </div>
              </div>
